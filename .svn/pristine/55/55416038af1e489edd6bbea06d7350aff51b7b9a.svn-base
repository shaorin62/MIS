...
if (!nexacro.Calendar) {
	...
	_pCalendar.on_apply_style_rtlimagemirroring = function (v) {
		...
		var popupcalendar = this.popupcalendar;
		if (popupcalendar) {
			popupcalendar.on_apply_style_rtlimagemirroring(v);
		}
	};
	...
	_pCalendar.on_notify_onspinup = function (obj, e) {
		...
		var add_info = this._makeValueStrObj(date, true);
		this._setPostValueAndText(this._makeDateObj(date, add_info));
	...
	_pCalendar._makeNormalValue = function (edit_value) {
		...
		var millisec = _info._millisec ? edit_value.substr(_info._millisec.index, _info._millisec.length).padLeft(3, "0") : "000";
	...
	_pCalendar._makeValueStrObj = function (edit_str, _is_fullvalue) {
		...
		for (i = 0, n = valueinfo_list.length; i < n; i++) {
			...
			if (!editinfo_list[list_idx]) {
				if (value_mask.charAt(0) == 'M') {
					...
					if (_is_fullvalue) {
						...
						add_mon = preval_str ? preval_str.substr(diff_len, 2) : "00";
					}
				}
					...
					if (_is_fullvalue) {
						...
						add_day = preval_str ? preval_str.substr(diff_len, 2) : "00";
					}
				}
				...
				if (value_mask.charAt(0) != edit_mask.charAt(0)) {
					...
					if (!empty_year && value_mask.charAt(0) == 'y' && value_mask.length > edit_mask.length) {
						...
						if (_is_fullvalue) {
							...
							var post_year = this._postvalue ? nexacro._toString(this._postvalue.getFullYear()) : preval_str;
							diff_len = value_mask.length - edit_mask.length;
							add_year = (post_year && post_year.length > 0) ? post_year.substr(0, diff_len) : "20";
						}
					}
	...
	_pCalendarCtrl._setValueCtrl = function (fire_event) {
		...
		if (edit && edit.value != undefined) {
			...
			if (focus_value != current_value && focus_value != current_value.trim()) {
				...
				var pre_value = this._makeDateObj(focus_value);
				var post_value = this._makeDateObj(value, add_info);

				this._setPreValueAndText(pre_value);
				this._setPostValueAndText(post_value, add_info);
