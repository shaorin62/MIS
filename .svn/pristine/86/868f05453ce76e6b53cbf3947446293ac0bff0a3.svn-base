...
if (!nexacro.Grid) {
	...
	_pGridTextArea.on_find_CurrentStyle_align = function (pseudo) {
		...
		if (cellobj) {
			...
			var align = nexacro.Component._default_textarea_align;
			return align;
		}
		return null;
	};
	...
	_pGrid._on_focus = function (self_flag, evt_name, lose_focus, refer_lose_focus, new_focus, refer_new_focus) {
		...
		if (this._showEditing) {
			this._currentCellEditor.on_apply_custom_setfocus();
			return;
		}
		var retn = false;
	...
	_pGrid._applySelect = function (arrS, arrE, pos) {
		...
		if (arrS.length == 0 || (each && pos < 0)) {
			...
			if (each) {
				for (var i = 0, n = nobodys.length; i < n; i++) {
			...
			if (nexacro._isArray(row)) {
				...
				for (var i = rowLen - 1; i >= 0; i--) {
					...
					if (!exist) {
						...
						for (var j = 0, rows_len = rows.length; j < rows_len; j++) {
							if (rows[j] == row[i]) {
								rows.splice(j, 1);
								break;
							}
	...
	_pGrid._exeFuncQueue = function (work) {
		for (var i = 0; i < this._func_queue; i++) {
