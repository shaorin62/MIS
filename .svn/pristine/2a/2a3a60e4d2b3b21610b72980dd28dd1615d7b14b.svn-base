CREATE OR REPLACE FUNCTION UWZ."SF_YEA_MASTXM" 
(
     iv_YEA_EMP_NO IN VARCHAR2
    ,iv_YEA_EMP_YMD IN VARCHAR2 DEFAULT TO_CHAR(SYSDATE, 'YYYYMMDD')
)
RETURN TTYPE_YEA_MASTXM AS

  v_tab TTYPE_YEA_MASTXM := TTYPE_YEA_MASTXM();
  /******************************************************************************
   NAME    :  SF_YEA_MASTXM
...
BEGIN
    FOR CUR IN (
        SELECT A.EMPL_NUMB AS YEA_EMP_NO   /*YEA_사원번호*/
             ...
             , A.WODN_CODE AS YEA_INOFFICE_CD   /*YEA_재직구분코드*/
          ...
          FROM HR_MASTXM A
            LEFT JOIN (
              SELECT * FROM (
                SELECT
                  ROW_NUMBER() OVER (PARTITION BY A.EMPL_NUMB ORDER BY A.APNT_DATE DESC, A.APNT_SEQN DESC) AS R_NUM
                 , A.*
                FROM HR_APPOXD A
          ...
          WHERE A.EMPL_NUMB = iv_YEA_EMP_NO
    )
      LOOP
        v_tab.extend;
        v_tab(v_tab.last) := RTYPE_YEA_MASTXM
