/**
 * core.erp.mac.service.impl.MACA0040ServiceImpl.java - <Created by Code Template generator>
 */
...
/**
 * <pre>
 *	MACA0040ServiceImpl - 공중파 수수료 세금계산서 서비스 인터페이스 구현 클래스
 * </pre>
 *
 * @author	최제현
 * @since	2016.11.21
 * @version	1.0
 * @see		{@link MACA0040Service}
 *
 * <pre>
...
@Service("MACA0040Service")
public class MACA0040ServiceImpl extends EgovAbstractServiceImpl implements MACA0040Service {

	/**
	 * 세금계산서 DAO class inject.
	 */
	@Resource(name = "MACA0040Dao")
	private MACA0040Dao dao;
	...
	public int processSAVE00(Map searchVo,Map saveData) throws Exception{
		...
		for (int i = 0; i < dsMD_TCALXM.size(); i++) {
			...
			String New_TAXX_NUMB=dao.processNew_TAXX_NUMB(rowVo);
			...
			//개별청약서에 세금계산서 번호를 기입
			if(rowVo.get("TABLE_NAME").toString().equals("MD_COMIXM")){
				dao.updateCMTX_NUMB(rowVo);
			}else{
				dao.updateCMTX_NUMB00(rowVo);
			}
	...
	public int processSAVE01(Map searchVo,Map saveData) throws Exception{
		...
		for (int i = 0; i < dsMD_TCALXM.size(); i++) {
			...
			if(overLap){
				String TAXX_NUMB=dao.processTAXX_NUMB(rowVo);
	...
	public int processDELETE00(Map saveData) throws Exception{
        ...
        for (int i = 0; i < dsMD_TCALXM.size(); i++) {
        	...
        	if(SACH_NUMB != null){
    			...
    			//수수료 테이블에 세금계산서 번호를 삭제
    			dao.updateCMTX_NUMB_Del(rowVo);			
    			dao.updateCMTX_NUMB_Del00(rowVo);
        	}
	...
	/**
	 * 세금계산서 디테일 MEMO_FILD,부가세 내용수정.영수구분수정
	 * @param param - 저장할 정보가 담긴 Map 객체
	 * @return 처리건수
	 * @exception Exception 예외
	 */
