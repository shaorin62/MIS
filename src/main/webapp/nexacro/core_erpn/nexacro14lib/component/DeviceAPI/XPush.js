...
if ((!nexacro.Device || nexacro.OS == "Android") && !nexacro.XPush) {
	if (nexacro.Browser != "Runtime" && !nexacro._isHybrid()) {
		...
		_pSockJSWrapper._nexacro_command = function (strCommand, strMessageType, strMessageKey, strType) {
			var cmd;
		...
		_pWebPush.command = function (strCommand, strMessageType, strMessageKey, strType) {
			if (!this._is_connected || !this.sockjs || parseInt(this.xpush.controlretry) < this.control_retry_cnt) {
				...
				this.xpush._onerror(status_code, conv_action, strCommand, strMessageType, strMessageKey);
				return;
			}
			...
			if (strCommand == "keepalive") {
				...
				this.sockjs._nexacro_command(strCommand, strMessageType, strMessageKey, strType);
			}
		};
		...
		_pWebPush._message = function (msg_data) {
			...
			if (msg_data.Action == "BYEC") {
				...
				if (msg_data.Action == this.action && 
					...
					this.command(this.action, this.msg_type, this.msg_key, this.type);
				}
			}
		};
	...
	_pXPush.command = function (strCommand, strMessageType, strMessageKey, objForm, objDataset, strType, strCallBack, nRow, strCheck, bUseActiveFormCallBack) {
		...
		if (this.strCommand === undefined || this.strCommand === null || this.strCommand == "") {
			...
			if (strCommand == "ADDF" || strCommand == "DELF") {
				...
				this.commandlist.push(cc);
				if (this._isWebPush()) {
					this.webpush.command(strCommand, strMessageType, strMessageKey);
				}
				else {
					if (this._handle) {
