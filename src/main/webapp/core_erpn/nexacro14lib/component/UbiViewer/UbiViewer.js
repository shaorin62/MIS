if (!nexacro.UbiViewer) {
	...
	nexacro.UbiViewer = function(id, position, left, top, width, height, right, bottom, parent) {
		...
		this.progress = "true";
		this.printsetmode = "0";
		this.exportsetmode = "0";
		this.gatewayProtocolType = "1";
...
//		this.callback_id = "";
		...
		this.excelSkipMasterItem = false;
		this.exceladjustpage = false;
		this.excelPrintPaperSize = "";
		this.excelPrintfitWidth = "";
		this.excelPrintfitHeight = "";
		// 사용자 폰트
		this.fontElement = "";
		// 보이스아이 설정
	...
	_pUbiViewer.execRetrieve = function(parent, callback, strUrl, isCallUrl) {
		...
		if (nexacro.Browser == "Runtime" && isCallUrl) {
			return;
		}
...
//					plugin_elem._plugin_object.callMethod("SetVariable", "useTobeCSV", "true");
					...
					plugin_elem._plugin_object.setProperty("IsDeflater", nexacro._toBoolean(this.isdeflater));
					plugin_elem._plugin_object.callMethod("SetVariable", "isBase64", ""+this.isbase64);
					plugin_elem._plugin_object.setProperty("IsUnicode", nexacro._toBoolean(this.isunicode));
					plugin_elem._plugin_object.setProperty("UtfData", nexacro._toBoolean(this.utfdata));
...
*/
	...
	_pUbiViewer.on_created_contents = function() {
		...
		else
		{
			...
			*/
			divTarget.innerHTML = "<div id='ubiviewer_" + this.divid_seq
			...
			this.viewer = new UbiViewer({
				...
				resid : this.resid,
				arg : this.arg,
				divid : "ubiviewer_" + this.divid_seq,
				scale : this.scale,
