...
if (!nexacro.Tab) {
	...
	_pTab.on_update_style_rtlimagemirroring = function () {
		...
		var extbtn = null;
		var len = this._tabButtons.length;
		var rtlimagemirroring = this.style.rtlimagemirroring;
		for (var i = 0; i < len; i++) {
			tabbtn = this._tabButtons[i];
			tabbtn.on_apply_style_rtlimagemirroring(rtlimagemirroring);
			extbtn = this._tabextrabuttons[i];
			if (extbtn) {
				extbtn.on_apply_style_rtlimagemirroring(rtlimagemirroring);
			}
		}

		if (this.spinupbutton) {
			this.spinupbutton.on_apply_style_rtlimagemirroring(rtlimagemirroring);
		}
		if (this.spindownbutton) {
			this.spindownbutton.on_apply_style_rtlimagemirroring(rtlimagemirroring);
		}
	};
