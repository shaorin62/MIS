	...
	_pFormBase.addIncludeScript = function (mainurl, url) {
		if (url) {
			...
			for (var i = 0, n = list.length; i < n; i++) {
				if (nexacro._compare(val, list[i])) {
					return;
				}
			}
			list.push(val);
		...
		if (nexacro.OS == "iOS" || (nexacro.Browser == "IE" || nexacro.Browser == "Edge" || nexacro.Browser == "MobileSafari")) {
			...
			if (comp && (comp._has_inputElement || comp instanceof nexacro.Grid)) {
				...
				if (edit_base && !edit_base._is_composition()) {
					if (nexacro.OSVersion >= 10 && (nexacro.Browser == "Edge" || nexacro.Browser == "IE") && (comp instanceof nexacro.TextArea)) {
						edit_base.comp._input_element._refreshElement1();
